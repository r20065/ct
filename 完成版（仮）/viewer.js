...
const axialDiv = document.getElementById("axial");
cornerstone.enable(axialDiv);
...
document.getElementById("folderInput").addEventListener("change", async e => {
    ...
    spacingX = ps[1]; spacingY = ps[0];
    spacingZ = slices > 1 ? Math.abs(parseFloat(loadedImages[1].data.string('x00200032').split('\\')[2]) - parseFloat(loadedImages[0].data.string('x00200032').split('\\')[2])) : 1.0;
    ...
    loadedImages.forEach((img, z) => {
        ...
        for (let i = 0; i < pixels.length; i++) volume[z * rows * cols + i] = pixels[i] * slope + intercept;
    });
...
function initSliders() {
    ...
    const setupSlider = (id, valId, max, val) => {
        const el = document.getElementById(id);
        el.max = max; el.value = val;
        document.getElementById(valId).textContent = val;
    };
    setupSlider('zSlider', 'zVal', slices - 1, zIndex);
    setupSlider('ySlider', 'yVal', rows - 1, yIndex);
    setupSlider('xSlider', 'xVal', cols - 1, xIndex);
...
function drawAxial() {
    ...
    const viewport = cornerstone.getViewport(axialDiv) || cornerstone.getDefaultViewportForImage(axialDiv, img);
    viewport.voi.windowWidth = ww;
    viewport.voi.windowCenter = wl;
    cornerstone.displayImage(axialDiv, img, viewport);

    setTimeout(() => {
        const canvas = axialDiv.querySelector('canvas');
